...
class MBConv(nn.Module):
    ...
    def __init__(
        ...
        if not downscale:
            ...
            ), "If downscaling is utilized input and output channels must be equal."
            ...
            *[
                MaxViTBlock(
                    ...
                    drop_path=(
                        ...
                        else drop_path[index]  # type:ignore
                    ),
                    mlp_ratio=mlp_ratio,
                    act_layer=act_layer,
                    norm_layer=norm_layer,
...
def max_vit_tiny_224(**kwargs) -> MaxViT:
    ...
    return MaxViT(
        depths=(2, 2, 5, 2), channels=(64, 128, 256, 512), embed_dim=64, **kwargs
    )
...
def max_vit_small_224(**kwargs) -> MaxViT:
    ...
    return MaxViT(
        depths=(2, 2, 5, 2), channels=(96, 128, 256, 512), embed_dim=64, **kwargs
    )
...
def max_vit_large_224(**kwargs) -> MaxViT:
    ...
    return MaxViT(
        depths=(2, 6, 14, 2), channels=(128, 256, 512, 1024), embed_dim=128, **kwargs
    )
...
if __name__ == "__main__":
    ...
    def test_partition_and_revers() -> None:
        ...
        windows = window_reverse(
            windows=windows,
            window_size=(7, 7),
            original_size=input.shape[2:],  # type:ignore
        )
        print(torch.all(input == windows))
        grid = grid_partition(input=input)
        grid = grid_reverse(
            grid=grid, grid_size=(7, 7), original_size=input.shape[2:]  # type:ignore
        )
    ...
    def test_transformer_block() -> None:
        transformer = MaxViTTransformerBlock(
            in_channels=128,
            partition_function=grid_partition,
            reverse_function=grid_reverse,
        )
